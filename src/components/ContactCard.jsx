...
const ContactCard = (props) => {
    const { userId, firstName, middleName, lastName, emailId, mobileNo, validFlag } = props.contact;
    return(
        <div className="flex items-center justify-between bg-white p-4 mb-4 rounded-lg shadow-md">
            ...
            <Link to={{pathname: `/updateContactDetails/${userId}`}}>
                <button 
            ...
            {validFlag === "Y" ? (
                <button 
                    className="text-red-500 border border-red-500 hover:bg-red-500 hover:text-white text-xl cursor-pointer bg-white px-4 py-2 rounded-lg" 
                    onClick={() => props.deleteHandler(userId)}>
                    <i className="fa-solid fa-trash-can mr-2"></i>Delete
                </button>
            ) : (
                <button 
                    className="text-blue-500 border border-blue-500 hover:bg-blue-500 hover:text-white text-xl cursor-pointer bg-white px-4 py-2 rounded-lg"
                    onClick={() => props.restoreHandler(userId)}>
                    <i className="fas fa-trash-restore mr-2"></i>Restore
                </button>
            )}
        </div>
    );
